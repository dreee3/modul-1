...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $bursa_kerja = $_POST['bursa_kerja'];
        $penelusuran = $_POST['penelusuran'];
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                    ...
                    $sql = "UPDATE alumni SET nama='$nama', tahun_lulus='$tahun_lulus', jurusan='$jurusan', bursa_kerja='$bursa_kerja', penelusuran='$penelusuran', foto='$target_file' WHERE id=$id";
            ...
            $sql = "UPDATE alumni SET nama='$nama', tahun_lulus='$tahun_lulus', jurusan='$jurusan', bursa_kerja='$bursa_kerja', penelusuran='$penelusuran' WHERE id=$id";
            if ($conn->query($sql) === TRUE) {
                echo "<div class='alert alert-success'>Data berhasil diperbarui.</div>";
            } else {
                echo "<div class='alert alert-danger'>Error: " . $sql . "<br>" . $conn->error . "</div>";
...
<div class="container mt-5">
    ...
    <?php if (isset($row)) { ?>
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="bursa_kerja" class="form-label">Bursa Kerja</label>
                <input type="text" class="form-control" id="bursa_kerja" name="bursa_kerja" value="<?php echo $row['bursa_kerja'] ?? ''; ?>" required>
            </div>

            <div class="mb-3">
                <label for="penelusuran" class="form-label">Penelusuran Alumni</label>
                <input type="text" class="form-control" id="penelusuran" name="penelusuran" value="<?php echo $row['penelusuran'] ?? ''; ?>" required>
            </div>
